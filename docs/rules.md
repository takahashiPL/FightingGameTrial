...
処理順は 移動 → Push 補正 → Facing。接触後の押し分けは双方等分補正の**暫定仕様**。
Push / Hurt / Hit Box の **Game ビュー可視化**も**実装済み**である（段階11A）。
Hit 判定自体はなお距離＋向きの暫定であり、Hurt/Hit の重なり判定置換は段階11B。
...
Unity デバッグ実装の**実際の到達点**（段階1〜11A）と次工程は
`docs/unity_implementation_status.md` を正とする。
（距離＋向きの Hit は暫定。Box 可視化は段階11Aで実装済み。重なり判定置換は段階11B。）
